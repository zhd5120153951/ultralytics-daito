...
def task_callback(action, task_id, task_name, status, message, create_time):
    ...
    Args:
        ...
        create_time: 创建时间
    """
    rds = Redis(host=redis_ip, port=redis_port,
    ...
    rds.xadd(train_action_result_topic_name, {
...
def generate_train_script(task_id, task_name, net_type, train_type, model_id, parameters):
    ...
    with open(script_path, 'w', encoding='utf-8') as file:
        ...
        # 连接Redis
        file.write(
            f'rds = redis.Redis("{redis_ip}", {redis_port}, decode_responses=True)\n\n')

        # 设置模型配置和路径
        if train_type == 'Init':  # 初始化训练
            if net_type.startswith('yolov5'):
                file.write(
                    f"model_cfg = '{pretrained_models}/{net_type}u.yaml'\n")
                file.write(
                    f"model_path = '{pretrained_models}/{net_type}u.pt'\n")
            else:
                file.write(
                    f"model_cfg = '{pretrained_models}/{net_type}.yaml'\n")
                file.write(
                    f"model_path = '{pretrained_models}/{net_type}.pt'\n")
        else:  # 迭代训练
...
def start_training_process(task_id, task_name, net_type, train_type, model_id, parameters, labels):
    ...
    script_path = generate_train_script(
        task_id, task_name, net_type, train_type, model_id, parameters)
    ...
    # 设置训练进度初始状态
    rds.hset(f'{process_key}_train_progress',
             mapping={'status': 1, 'epoch': 0})
    ...
    # 创建监控线程
    monitor_thread = Thread(target=monitor_training_process, args=(
        process_key, labels, parameters))
...
def monitor_training_process(process_key, labels, parameters):
    ...
    for line in process.stdout:
        ...
        if match_ret and match_ret.group(1):
            ...
            # 更新Redis中的训练进度
            rds.hset(f'{process_key}_train_progress', mapping={
                     'status': 1, 'epoch': current_epoch})
    ...
    # 删除Redis中的训练进度信息
    rds.hdel(f'{process_key}_train_progress', *
             rds.hkeys(f'{process_key}_train_progress'))
    rds.close()
    ...
    try:
        ...
        if action == 'start':
            ...
            start_training_process(
                task_id, task_name, net_type, train_type, model_id, parameters, labels)
...
def main():
    ...
    # 创建消费者组
    try:
        rds.xgroup_create(train_action_opt_topic_name,
            ...
            if messages:
                for stream, message_list in messages:
                    for message_id, message in message_list:
                        ...
                        # 确认消息
                        rds.xack(train_action_opt_topic_name,
